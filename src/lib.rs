    ...
    async fn from_request(req: Request, state: &S) -> Result<Self, Self::Rejection> {
        ...
        match accept.as_bytes() {
            ...
            b"application/msgpack" => {
                ...
                let body = rmp_serde::from_slice(&body).map_err(|e| {
        ...
        Box::pin(async move {
            ...
            let body = match encoding {
                ...
                "application/msgpack" => {
                    ...
                    {
                        let mut serializer =
                            rmp_serde::Serializer::new(&mut body).with_struct_map();
                        let mut serializer = <dyn erased_serde::Serializer>::erase(&mut serializer);
                        if let Err(e) = payload.erased_serialize(&mut serializer) {
                            tracing::error!(error = %e, "failed to deserialize request body as msgpack");
    ...
    mod msgpack {
        ...
        mod input {
            ...
            async fn test_can_read_input_with_specified_header() {
                ...
                let app = Router::new().route("/", post(handler));
                let body = {
                    let body = HashMap::from([("message", "test")]);
                    rmp_serde::encode::to_vec_named(&body).unwrap()
                };
        ...
        mod output {
            ...
            async fn test_encode_as_requested() {
                ...
                let expected_body = {
                    let body = HashMap::from([("message", "Hello, test!")]);
                    rmp_serde::encode::to_vec_named(&body).unwrap()
                };
